...
public class FBDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		List<FbNodeDescriptor> childDescriptors = FbDiagramUpdater
				.getFBDiagram_1000SemanticChildren(viewObject);
	...
	protected void refreshSemantic() {
		...
		List<FbNodeDescriptor> childDescriptors = FbDiagramUpdater
				.getFBDiagram_1000SemanticChildren((View) getHost().getModel());
		...
		for (Iterator<FbNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			FbNodeDescriptor next = descriptorsIterator.next();
			String hint = FbVisualIDRegistry.getType(next.getVisualID());
		...
		for (FbNodeDescriptor next : childDescriptors) {
			String hint = FbVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<FbLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = FbVisualIDRegistry
			...
			for (Iterator<FbLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				FbLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<FbLinkDescriptor> collectAllLinks(View view,
		...
		case FBDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(FbDiagramUpdater
						.getFBDiagram_1000ContainedLinks(view));
			}
		...
		case OUTVariableEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(FbDiagramUpdater
						.getOUTVariable_3003ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case INVariableEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(FbDiagramUpdater
						.getINVariable_3002ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	private Collection<IAdaptable> createConnections(
			Collection<FbLinkDescriptor> linkDescriptors,
			Domain2Notation domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (FbLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					FbVisualIDRegistry.getType(nextLinkDescriptor.getVisualID()),
	...
	protected final EditPart getHintedEditPart(EObject domainModelElement,
		...
		View view = (View) domain2NotationMap.getHinted(domainModelElement,
				FbVisualIDRegistry.getType(hintVisualId));
		if (view != null) {
			return (EditPart) getHost().getViewer().getEditPartRegistry()
					.get(view);
		}
